...
void Blip_Buffer::end_frame( blip_time_t t ) {
	offset_ += t * factor_;
	assert(samples_avail() <= (long) buffer_size_); // Blip_Buffer::end_frame(): Frame went past end of buffer
}

void Blip_Buffer::remove_silence( long count ) {
	assert(count <= samples_avail()); // Blip_Buffer::remove_silence(): Tried to remove more samples than available
