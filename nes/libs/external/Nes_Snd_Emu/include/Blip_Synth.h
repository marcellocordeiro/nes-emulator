...
inline constexpr int blip_low_quality  = 1;
inline constexpr int blip_med_quality  = 2;
inline constexpr int blip_good_quality = 3;
inline constexpr int blip_high_quality = 4;
...
template <int quality, int range> class Blip_Synth {
  ...
  static constexpr int width =
      (quality < 5 ? quality * 4 : Blip_Buffer::widest_impulse_);
  ...
  static constexpr int fine_bits =
      (fine_mode ? (abs_range <= 64
                        ? 2
                        : abs_range <= 128
                              ? 3
                              : abs_range <= 256
...
public:
  ...
  // Configure low-pass filter (see notes.txt). Not optimized for real-time
  // control
  ...
  // Add an amplitude offset (transition) with a magnitude of delta *
  // volume_unit into the specified buffer (default buffer if none specified) at
  // the specified source time. Delta can be positive or negative. To increase
  // performance by inlining code at the call site, use offset_inline().
  void offset(blip_time_t, int delta, Blip_Buffer*) const;

  void offset_resampled(Blip_Buffer::resampled_time_t, int delta,
                        Blip_Buffer*) const;
  void offset_resampled(Blip_Buffer::resampled_time_t t, int o) const
  {
    offset_resampled(t, o, impulse.buf);
  }
  void offset(blip_time_t t, int delta) const { offset(t, delta, impulse.buf); }
...
template <int quality, int range> class Blip_Wave {
...
inline void Blip_Synth<quality, range>::offset_resampled(
    Blip_Buffer::resampled_time_t time, int delta, Blip_Buffer* blip_buf) const
{
  ...
  assert(
      sample_index <
      blip_buf->buffer_size_);  // Blip_Synth/Blip_wave: Went past end of buffer
...
template <int quality, int range>
void Blip_Synth<quality, range>::offset(blip_time_t time, int delta,
