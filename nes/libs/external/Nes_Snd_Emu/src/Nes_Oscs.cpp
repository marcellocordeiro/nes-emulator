...
void Nes_Envelope::clock_envelope()
{
  ...
  if (reg_written[3]) {
    ...
    env_delay = period;
    if (envelope | (regs[0] & 0x20)) envelope = (envelope - 1) & 15;
  }
}
...
void Nes_Square::clock_sweep(int negative_adjust)
{
  ...
  if (--sweep_delay < 0) {
    ...
    if (shift && (sweep & 0x80) && period >= 8) {
      ...
      if (sweep & negate_flag) offset = negative_adjust - offset;
...
void Nes_Square::run(cpu_time_t time, cpu_time_t end_time)
{
  ...
  if (regs[1] & negate_flag) offset = 0;
  ...
  if (volume == 0 || period < 8 || (period + offset) >= 0x800) {
    ...
    if (phase < duty) amp ^= volume;

    int delta = update_amp(amp);
    if (delta) synth->offset(time, delta, output);
...
void Nes_Triangle::clock_linear_counter()
{
  ...
  if (!(regs[0] & 0x80)) reg_written[3] = false;
}
...
int Nes_Dmc::count_reads(cpu_time_t time, cpu_time_t* last_read) const
{
  ...
  if (length_counter == 0) return 0;  // not reading
  ...
  if (!(regs[0] & loop_flag) && count > length_counter) count = length_counter;

  if (last_read) {
    *last_read = first_read + (count - 1) * (period * 8) + 1;
    assert(*last_read <= time);
    assert(count == count_reads(*last_read, nullptr));
    assert(count - 1 == count_reads(*last_read - 1, nullptr));
  }
...
static const short dmc_period_table[2][16] = {
    ...
    {0x18e, 0x161, 0x13c, 0x129, 0x10a, 0x0ec, 0x0d2,
     0x0c7,  // PAL (totally untested)
     0x0b1, 0x095, 0x084, 0x077, 0x062, 0x04e, 0x043,
...
static const unsigned char dac_table[128] = {
    0,  0,  1,  2,  2,  3,  3,  4,  5,  5,  6,  7,  7,  8,  8,  9,  10, 10, 11,
    11, 12, 13, 13, 14, 14, 15, 15, 16, 17, 17, 18, 18, 19, 19, 20, 20, 21, 21,
    22, 22, 23, 23, 24, 24, 25, 25, 26, 26, 27, 27, 28, 28, 29, 29, 30, 30, 31,
    31, 32, 32, 32, 33, 33, 34, 34, 35, 35, 35, 36, 36, 37, 37, 38, 38, 38, 39,
    39, 40, 40, 40, 41, 41, 42, 42, 42, 43, 43, 44, 44, 44, 45, 45, 45, 46, 46,
    47, 47, 47, 48, 48, 48, 49, 49, 49, 50, 50, 50, 51, 51, 51, 52, 52, 52, 53,
    53, 53, 54, 54, 54, 55, 55, 55, 56, 56, 56, 57, 57, 57};
...
void Nes_Dmc::fill_buffer()
{
  if (buf_empty && length_counter) {
    assert(rom_reader);  // rom_reader must be set
...
void Nes_Noise::run(cpu_time_t time, cpu_time_t end_time)
{
  ...
  if (time < end_time) {
    ...
    if (!volume) {
      ...
      Blip_Buffer::resampled_time_t rperiod =
