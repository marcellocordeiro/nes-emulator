...
void Blip_Buffer::clear(bool entire_buffer)
{
  ...
  offset_      = 0;
  reader_accum = 0;
  memset(buffer_, sample_offset & 0xFF,
         (count + widest_impulse_) * sizeof(buf_t_));
}

void Blip_Buffer::sample_rate(long new_rate, int msec)
{
  unsigned new_size =
      (UINT_MAX >> BLIP_BUFFER_ACCURACY) + 1 - widest_impulse_ - 64;
  if (msec != blip_default_length) {
    unsigned s = (new_rate * (msec + 1) + 999) / 1000;
    if (s < new_size)
      new_size = s;
    else
  ...
  if (buffer_size_ != new_size) {
    delete[] buffer_;
    buffer_      = nullptr;  // allow for exception in allocation below
    buffer_size_ = 0;
    offset_      = 0;
  ...
  if (msec) assert(length_ == msec);  // ensure length is same as that passed in

  samples_per_sec = new_rate;
  if (clocks_per_sec) clock_rate(clocks_per_sec);  // recalculate factor
...
{
  clocks_per_sec = cps;
  factor_        = (unsigned long)floor(
      (double)samples_per_sec / cps * (1L << BLIP_BUFFER_ACCURACY) + 0.5);
...
void Blip_Buffer::bass_freq(int freq)
{
  ...
  bass_shift =
      1 + (int)floor(1.442695041 * log(0.124 * samples_per_sec / freq));
...
constexpr int  impulse_bits   = 15;
constexpr long impulse_amp    = 1L << impulse_bits;
constexpr long impulse_offset = impulse_amp / 2;
...
void Blip_Impulse_::scale_impulse(int unit, imp_t* imp_in) const
{
  ...
  if (res > 2) {
    ...
    for (int nn = (res / 2 - 1) * width - 1; nn--;) *imp++ = *--rev;
    *imp++ = (imp_t)unit;
  }
...
void Blip_Impulse_::treble_eq(const blip_eq_t& new_eq)
{
  ...
  for (int i = size; i--;) {
    ...
    double c = pow_a_n * rolloff * cos((n_harm - 1.0) * angle) -
               ...
               pow_a_nc * rolloff * cos_nc1_angle + pow_a_nc * cos_nc_angle;
  ...
  double factor =
      impulse_amp * 0.5 / total;  // 0.5 accounts for other mirrored half
  imp_t*    imp    = impulse;
...
void Blip_Buffer::remove_samples(long count)
{
  ...
  // to do: kind of hacky, could add run_until() which keeps track of extra
  // synthesis
                               ...
                               bool stereo)
{
  ...
  if (!count) return 0;  // optimization
...
{
  offset_ += t * factor_;
  assert(samples_avail() <=
         (long)buffer_size_);  // Blip_Buffer::end_frame(): Frame went past end
                               // of buffer
}

void Blip_Buffer::remove_silence(long count)
{
  assert(count <= samples_avail());  // Blip_Buffer::remove_silence(): Tried to
