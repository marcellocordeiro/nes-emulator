...
auto MainWindow::show() -> void
{
  ...
  window.reset(SDL_CreateWindow(Emulator::title.data(), SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
                                Emulator::width * 3, Emulator::height * 3, 0));

  renderer.reset(SDL_CreateRenderer(window.get(), -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC));

  SDL_RenderSetLogicalSize(renderer.get(), Emulator::width, Emulator::height);

  texture.reset(SDL_CreateTexture(renderer.get(), SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING,
...
{
  SDL_UpdateTexture(texture.get(), nullptr, buffer, Emulator::width * sizeof(uint32_t));
